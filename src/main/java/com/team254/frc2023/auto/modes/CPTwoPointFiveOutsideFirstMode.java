...
/*
 * Cable Protector/Bump Side - Scores High Cone, then picks up outside cube, scores it high, then picks up inside cube and balances on bridge.
 * */
public class CPTwoPointFiveOutsideFirstMode extends AutoModeBase {

    AutoModeSelector.DockMode mDockMode;
    public CPTwoPointFiveOutsideFirstMode(AutoModeSelector.DockMode dockMode) {
        mDockMode = dockMode;
        ...
        addAction(
                new ParallelAction(
                        new SeriesAction(
                                new SuperstructureAction(Superstructure.GoalState.SCORE, 0.1),
        ...
        if(mDockMode == AutoModeSelector.DockMode.DOCK) {
            DockActionNew dockAction = new DockActionNew(DockActionNew.StartingSide.FAR, DockActionNew.ApproachOrientation.BACK_FIRST, 1.25, 15);
            addAction(
                    new ParallelAction(
                            new SuperstructureAction(Superstructure.GoalState.STOW),
                            new InterruptableAction(new DriveTrajectoryAction(Drive.getInstance().getTrajectoryGenerator().getTrajectorySet().cpInsidePickupToDock, false), dockAction::getInclinationAboveThresholdForDock)
                    )
            );
